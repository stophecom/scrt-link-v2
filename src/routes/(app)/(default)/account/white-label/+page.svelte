<script lang="ts">
	...
	let currentStep = $derived(!hasDomain ? 0 : 1);
</script>
...
{#if data.userOrganization}
	...
	{/if}
{:else}
	<p class="text-muted-foreground mb-4 text-sm">{m.fresh_bad_midge_explore()}</p>
	<Button variant="outline" href={localizeHref('/account/organization')}
		>{m.wild_inner_fox_honor()}</Button
	>
{/if}
